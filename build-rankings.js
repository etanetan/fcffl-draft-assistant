...
[70,"Kenny Gainwell","TB","RB",6,""],[71,"Jaxson Dart","NYG","QB",3,""],[72,"Jaylen Warren","PIT","RB",6,""],
...
[82,"Sam LaPorta","DET","TE",3,""],[83,"Travis Hunter","JAX","WR",7,"plays DB — WR4"],[84,"Xavier Worthy","KC","WR",5,""],
...
// News-driven rank corrections. BASE is a static early-Sept consensus and cannot
// know about a suspension, an exempt-list placement, or a role change discovered
// in camp. Rather than editing BASE (and renumbering 200 rows by hand), override
// here and let the sort below reflow the board.
//   Travis Hunter: Sleeper lists him pos=DB, depth chart SWR#4; camp reps ran
//   41 CB / 28 WR. A part-time WR4 is not a top-100 asset.
const RANK_OVERRIDE = {
  "Travis Hunter": 168,
};

function derive(rank, name, team, pos, tier, note) {
  rank = RANK_OVERRIDE[name] ?? rank;
...
// An override can put a player out of order, and app.js renders in array order
// rather than sorting -- so reflow here, then renumber 1..N so ranks stay dense
// and unique (a duplicate rank would render as two identical "rk 168" rows).
const out = BASE.map(r => derive(...r)).sort((a, b) => a.rank - b.rank);
